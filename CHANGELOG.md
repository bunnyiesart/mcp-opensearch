...
- `opensearch_multi_terms` rejects an aggregation `id` beginning with `_`. That prefix is reserved for response metadata such as `_warning`, which shares the result dict with the aggregations — and because the id is caller-supplied, an agent passing `id="_warning"` received the warning string back *instead of* the aggregation it requested. Verified reachable. The response shape is unchanged: restructuring the output of `terms`, `multi_terms` and `discover_fields` would have broken every consumer in order to handle a name a caller can simply be told not to use. The residual collision in `terms` and `discover_fields` requires the *data itself* to contain a value or field name literally equal to `_warning`, and is documented rather than fixed.
...
### Fixed
- **`opensearch_search` presented a truncated hit count as an exact total.** OpenSearch stops counting at `track_total_hits` (10,000 by default) and signals it via `hits.total.relation: "gte"`; that field was discarded, so a floor was returned as though it were a count. Measured against a live cluster: a wildcard search reported `total: 10000` where `opensearch_count` returned **33,645,389** — a 3,364× under-report on the question "how many events match?". `total` is unchanged, but a truncated count now carries an explicit lower-bound warning pointing at `opensearch_count`, which is not subject to the cap. Same fix applied to `opensearch_get_anomaly_results`. Found by running the server against a real cluster — every unit-test stub emitted `relation: "eq"`, so the fixtures only ever described the case that worked.
